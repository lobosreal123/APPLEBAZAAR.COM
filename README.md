...
5. **Telegram on new orders (optional, from website)**
   ...
   - POS app (`pos-system`) does **not** send Telegram for new website orders; only **this** storefront does (`Checkout.tsx` + `telegramNotify.ts`), so you avoid duplicate messages.
